...
export function shapesFor(paper: Paper, pen: Pen): readonly Shape[] {
  const { cols, rows } = gridSizeFor(paper, pen.pitch)
  return shapeLibrary((rows * pen.pitch) / (cols * pen.pitch))
}
...
export function generateMaze(settings: MazeSettings): GeneratedMaze {
  const { paper, pen, level, shape, seed } = settings

  const { cols, rows } = gridSizeFor(paper, pen.pitch)
  const grid = new MaskedGrid(new SquareGrid(cols, rows, pen.pitch), shape.mask)
